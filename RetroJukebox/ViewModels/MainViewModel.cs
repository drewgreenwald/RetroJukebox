...
public partial class MainViewModel : ObservableObject
{
    ...
    private readonly SavedPlaylistService _savedPlaylists;
    ...
    // ── Phase 7: named playlists ──────────────────────────────────────────
    public ObservableCollection<SavedPlaylist> SavedPlaylists => _savedPlaylists.Playlists;

    /// <summary>Controls which panel is shown in the right column: "Queue" or "Playlists".</summary>
    [ObservableProperty] private string _rightPanelTab = "Queue";
...
#pragma warning restore CS8618
    {
        ...
        _savedPlaylists = App.SavedPlaylistService;
    ...
    // ── Phase 7: named playlist commands ─────────────────────────────────

    [RelayCommand]
    private void SaveQueueAsPlaylist(string name)
    {
        if (string.IsNullOrWhiteSpace(name)) return;
        _savedPlaylists.Save(name, _playlist.Queue);
        StatusText = $"Saved playlist \"{name}\"";
    }

    [RelayCommand]
    private void LoadPlaylist(SavedPlaylist playlist)
    {
        var tracks = _savedPlaylists.Resolve(playlist);
        if (tracks.Count == 0)
        {
            StatusText = $"Playlist \"{playlist.Name}\" has no resolvable tracks.";
            return;
        }
        _playlist.Clear();
        _playlist.EnqueueRange(tracks);
        RightPanelTab = "Queue";
        StatusText = $"Loaded \"{playlist.Name}\" — {tracks.Count} tracks";
    }

    [RelayCommand]
    private void DeletePlaylist(SavedPlaylist playlist)
    {
        _savedPlaylists.Delete(playlist);
        StatusText = $"Deleted playlist \"{playlist.Name}\"";
    }

    [RelayCommand]
    private void SwitchRightPanel(string tab) => RightPanelTab = tab;
